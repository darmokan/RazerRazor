...
$logFile = "$env:USERPROFILE\Desktop\RazerRazorLog.txt"
...
Write-Host "`n=== Razer™ Razor Cleanup Script ===" -ForegroundColor Cyan
...
Write-Host "Any and all references to Razer™, Inc. and its copyrights are COPYRIGHT © 2026 Razer INC. ALL RIGHTS RESERVED."
...
if (Ask-User "Stop all Razer™ processes and services?") {
    # Services (Must use wildcards without ™ because the actual service name is just "Razer")
...
# Note: Do not use ™ in paths; the folder on disk is simply "Razer"
$paths = @(
